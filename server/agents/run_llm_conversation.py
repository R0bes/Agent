#!/usr/bin/env python3
"""
Eigenständiges Script für das LLM-Konversations-Experiment.
Kann direkt ausgeführt werden, um zwei LLMs miteinander sprechen zu lassen.
"""
...
try:
    ...
    print("Stelle sicher, dass du im richtigen Verzeichnis bist und alle Abhängigkeiten installiert sind.")
    sys.exit(1)
...
class ConversationAgent(OllamaAgent):
    ...
    async def chat_with_agent(self, message: str, other_agent_name: str) -> str:
        ...
        history_string = '\n'.join(
            [f"{msg['from']}: {msg['message']}" for msg in self.conversation_history[:-max(20, len(self.conversation_history))]]
        )
...
class LLMConversationExperiment:
        ...
        while (turn_count < self.max_turns) or self.endless:
            ...
            try:
                response = await other_agent.chat_with_agent(
                    ...
                    other_agent_name=current_speaker.name
                )
    ...
    def _print_conversation_summary(self):
        ...
        print(f"  {self.agent1.name} sprach: {len([log for log in self.conversation_log if log['speaker'] == self.agent1.name])} mal")
        print(f"  {self.agent2.name} sprach: {len([log for log in self.conversation_log if log['speaker'] == self.agent2.name])} mal")
