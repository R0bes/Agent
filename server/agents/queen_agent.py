...
class QueenConfig(OllamaConfig):
    ...
    default_system_prompt: str = (
        "Du bist die Queen - eine weise, freundliche und hilfreiche Assistentin. "
        "Du antwortest immer höflich und respektvoll. Du bist eine Expertin in vielen Bereichen "
        "und kannst komplexe Fragen verständlich erklären. Antworte auf Deutsch, es sei denn, "
...
class QueenAgent(OllamaAgent):
    ...
    async def chat_response(
        ...
        **kwargs
        ...
        try:
            ...
            self.queen_logger.info(f"Queen hat auf Nachricht von {user_id or 'unbekannt'} geantwortet")
    ...
    async def chat_response_stream(
        ...
        **kwargs
        ...
        Falls Streaming fehlschlägt, wird auf normale Antworten zurückgegriffen.
        """
        if not self.is_queen_active:
            raise Exception("Queen ist nicht aktiv")
        ...
        try:
            ...
            try:
                ...
                self._add_to_memory("assistant", complete_response, user_id, conversation_id)
                
            except Exception as stream_error:
                # Fallback auf normale Antworten
                self.queen_logger.warning(f"Streaming fehlgeschlagen, verwende Fallback: {stream_error}")
            ...
            self.queen_logger.info(f"Queen hat gestreamte Antwort für {user_id or 'unbekannt'} generiert")
    ...
    async def start_conversation(
        ...
        welcome_message = {
            "queen_name": "Queen",
            "message": f"Willkommen! Ich bin Queen, deine weise Assistentin. Wie kann ich dir heute helfen?",
            "conversation_id": conversation_id,
            "user_id": user_id,
    ...
    async def end_conversation(
        ...
        if not farewell_message:
            farewell_message = "Es war mir eine Freude, dir zu helfen! Bis zum nächsten Mal!"
    ...
    def _clear_conversation_memory(self, user_id: str, conversation_id: Optional[str] = None):
        ...
        if conversation_id:
            ...
            self.conversation_memory = [
                ...
                if not (msg.get("user_id") == user_id and msg.get("conversation_id") == conversation_id)
            ]
        else:
    ...
    def _enhance_system_prompt(self, user_message: str) -> str:
        ...
        base_prompt = self.config.system_prompt or "Du bist die Queen - eine weise, freundliche und hilfreiche Assistentin."
        ...
        context_enhancement = ""
        if "programmiere" in user_message.lower() or "code" in user_message.lower() or "python" in user_message.lower():
            context_enhancement = " Du bist auch eine Expertin für Programmierung und Software-Entwicklung."
        elif "wissenschaft" in user_message.lower() or "forschung" in user_message.lower() or "physik" in user_message.lower():
            context_enhancement = " Du bist auch eine Expertin für Wissenschaft und Forschung."
        
        enhanced_prompt = f"{base_prompt} {style_prompt} {context_enhancement}".strip()
    ...
    def __str__(self) -> str:
        return f"QueenAgent(name={self.name}, style={self.response_style}, active={self.is_queen_active})"
