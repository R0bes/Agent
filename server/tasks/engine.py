"""
Task Engine für die Verarbeitung von Nachrichten.
"""
...
class GlobalEventManager:
    ...
    def submit_message(self, message_data: dict, client_id: str) -> str:
        ...
        try:
            ...
            self.logger.debug(
                f"Nachricht von {client_id} zur Queue hinzugefügt: {message_event.event_id}"
            )
    ...
    def register_message_handler(
        self, message_type: str, handler: Callable[[MessageEvent], None]
    ) -> None:
        """
    ...
    async def _process_message(self, message_event: MessageEvent) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Fehler bei der Verarbeitung der Nachricht {message_event.event_id}: {e}"
            )
            self.stats["failed_messages"] += 1
...
class TaskEngine:
    ...
    async def submit_task(self, task: Task, task_input: TaskInput) -> str:
        ...
        try:
            ...
            self.logger.debug(
                f"Task {task.task_id} zur Queue hinzugefügt (Priorität: {task.priority.name})"
            )
            return task.task_id
    ...
    async def _execute_task(self, task: Task) -> None:
        ...
        try:
            ...
            future = self.executor.submit(self._run_task_sync, task, task.input)
    ...
    def _run_task_sync(self, task: Task, task_input: TaskInput) -> TaskOutput:
        ...
        try:
            ...
            return TaskOutput(result=None, success=False, error=str(e))
        finally:
