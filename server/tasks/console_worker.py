"""
Console Worker für Task-Verarbeitung.
"""
...
class ConsoleWorker:
    ...
    def _print_message_task_result(self, task: Task) -> None:
        ...
        if message_event.message_data.get("content"):
            print(
                f"Inhalt: {message_event.message_data['content'][:100]}{'...' if len(message_event.message_data['content']) > 100 else ''}"
            )
    ...
    def _print_generic_task_result(self, task: Task) -> None:
    ...
    def print_system_status(self, status_data: Dict[str, Any]) -> None:
        ...
        if "event_manager" in status_data:
            ...
            print(
                f"Event Manager: {'🟢 Läuft' if event_stats.get('is_running') else '🔴 Gestoppt'}"
            )
