...
import type { SourceInterface, SourceMessage, SourceKind, SourceDescriptor } from "../types";
import { eventBus } from "../../events/eventBus";
import { logInfo, logDebug, logError } from "../../utils/logger";

export abstract class AbstractSource implements SourceInterface {
  abstract readonly id: string;
  abstract readonly kind: SourceKind;
  abstract readonly label?: string;

  /**
   * Convert raw incoming payload into SourceMessages - must be implemented by subclasses
   */
  abstract toSourceMessages(raw: any): Promise<SourceMessage[]>;

  /**
   * Send a source message to the persona via event bus
   */
  protected async sendToPersona(message: SourceMessage): Promise<void> {
   ...
   */
  async process(raw: any): Promise<void> {
   ...
   */
  protected createSourceDescriptor(
    id: string,
    label?: string,
