...
class TestHTTPEndpoints:
    ...
    def client(self):
        ...
        # Temporär deaktiviert wegen httpx-Versionsproblemen
        pytest.skip("HTTP-Tests temporär deaktiviert - httpx-Versionsproblem")
    
    def test_root_endpoint_returns_api_info(self, client):
        """Testet, dass der Root-Endpoint API-Informationen zurückgibt."""
        pytest.skip("HTTP-Tests temporär deaktiviert")
...
class TestErrorHandling:
    ...
    @patch('server.api.get_queen_instance')
    def test_queen_agent_timeout_handling(self, mock_get_queen, client):
        """Testet Timeout-Behandlung bei Queen-Agent-Aufrufen."""
        import asyncio
        
        # Mock Queen Agent mit Timeout
        async def timeout_response():
            await asyncio.sleep(0.1)  # Simuliert Verzögerung
            raise asyncio.TimeoutError("Queen Agent Timeout")
        
        mock_queen = Mock()
        mock_queen.chat_response = timeout_response
        mock_get_queen.return_value = mock_queen
    ...
    def test_malformed_request_handling(self, client):
        ...
        response = client.post("/chat", data="raw data", headers={"Content-Type": "text/plain"})
...
class TestSecurity:
    ...
    def test_sql_injection_prevention(self, client):
        ...
        with patch('server.api.get_queen_instance') as mock_get_queen:
            ...
            mock_get_queen.assert_called_once()
