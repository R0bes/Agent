...
import { SchedulerTool } from "../../../tools/worker/schedulerTool";
/**
...
class SchedulerToolAdapter extends AbstractTool {
    constructor() {
        ...
        this.name = "scheduler";
        this.shortDescription = "Schedules and triggers background workers (e.g. memory workers) on behalf of the persona.";
        this.description = "Schedules and triggers background workers (e.g. memory workers) on behalf of the persona. This tool allows the persona to schedule background tasks like memory compaction, which can aggregate older messages into summaries or extract stable facts/preferences. Use this tool when you detect that memory clean-up or summarisation should happen in the background.";
        this.parameters = {
            type: "object",
            properties: {
                kind: {
    ...
    async execute(args, ctx) {
        return await this.schedulerTool.execute(args, ctx);
    }
}
 ...
 * Scheduler Tool Component
 */
export const schedulerToolComponent = {
    id: "scheduler-tool",
