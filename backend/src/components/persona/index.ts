...
class PersonaService extends AbstractService {
  readonly id = "persona-service";
  readonly name = "Persona Service";
  readonly description = "Core persona service that processes messages and coordinates with tools";
  ...
  async handleCall(call: { method: string; params: Record<string, unknown> }): Promise<{ success: boolean; data?: unknown; error?: string }> {
    if (call.method === "processMessage") {
      try {
        const sourceMessage = call.params.sourceMessage as SourceMessage;
