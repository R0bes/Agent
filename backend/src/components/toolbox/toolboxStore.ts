/**
 * Persistent Storage for Toolbox State
 * 
...
class ToolboxStore {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logInfo("Toolbox Store: State loaded from file", {
        toolCount: Object.keys(this.state.tools).length
      });
    } catch (err: any) {
      if (err.code === "ENOENT") {
        // File doesn't exist yet - that's fine, we'll create it on first save
        logDebug("Toolbox Store: No existing state file, starting fresh");
      } else {
        logWarn("Toolbox Store: Failed to load state file", {
          error: err.message
        });
      }
    }
  ...
  async enableTool(toolName: string): Promise<void> {
    ...
    await this.save();
    logInfo("Toolbox Store: Tool enabled", { toolName });
  }
  ...
  async disableTool(toolName: string): Promise<void> {
    ...
    await this.save();
    logInfo("Toolbox Store: Tool disabled", { toolName });
  }
  ...
  private async save(): Promise<void> {
    try {
      ...
      logDebug("Toolbox Store: State saved to file");
    } catch (err) {
      logError("Toolbox Store: Failed to save state file", err);
      throw err;
    }
  }
}
