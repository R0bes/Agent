...
export async function registerJobsRoutes(app: FastifyInstance) {
  app.get("/api/jobs", async (req, reply) => {
    ...
    const jobs = workerJobs.map((wj) => {
      // Get worker to determine category
      const worker = bullMQWorkerEngine.getWorker(wj.workerName);
      const category = worker?.category || (wj.workerName.includes("memory") ? "memory" : "tool");
      
      return {
        id: wj.id,
        workerName: wj.workerName,
        category: category,
        type: category === "memory" ? "memory" as const : category === "think" ? "think" as const : "tool" as const,
